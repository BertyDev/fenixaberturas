<div class=" container py-12">
    ...
    <x-jet-form-section submit="save" class="mb-6">
        ...
        <x-slot name="description">
            complete la información necesaria para poder agregar {{ __('Province') }}
        </x-slot>
        <x-slot name="form">
    ...
    <x-jet-action-section>
        <x-slot name="title">
            Listado de {{ __('Provinces') }}
        </x-slot>
        <x-slot name="description">
            Aquí encontrara todas las {{ __('Provinces') }} agregadas
        </x-slot>
        <x-slot name="content">
                    ...
                    @foreach ($departments as $department)
                        <tr>
                            <td class="py-2">
                                ...
                                <a  href="{{ route('admin.department.show',$department) }}"
                                class="underline hover:text-blue-600 uppercase">{{ $department->name }}</a>
                            </td>
                            <td class="py-2">
                                <div class="flex divide-x divide-gray-300 font-semibold"
                                    ...
                                    <a class="pr-2 hover:text-blue-600 cursor-pointer" wire:loading.attr='disabled'
                                        ...
                                        wire:click="edit('{{ $department->id }}')">{{ __('Edit') }}</a>

                                    <a wire:click="confirdepartmentDelete" wire:loading.attr='disabled'
                                        wire:target="confirdepartmentDelete"
    ...
    <x-jet-confirmation-modal wire:model="open_confir" maxWidth="md">
        ...
        <x-slot name="footer">
            ...
            <x-jet-danger-button wire:click="delete('{{ $department->id }}')" wire:loading.attr='disabled'
                wire:target="delete('{{ $department->id }}')">
                eliminar
            </x-jet-danger-button>
        </x-slot>
